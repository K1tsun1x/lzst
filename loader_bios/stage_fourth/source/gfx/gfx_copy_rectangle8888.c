...
void gfx_copy_rectangle8888(int src_x, int src_y, int dst_x, int dst_y, size_t width, size_t height) {
	...
	for (int row = row_start; row != row_end; row += row_step) {
		uint32_t* src_line = (uint32_t*)(GFX_BUFFER + (src_y + row) * vm->pitch) + src_x;
		uint32_t* dst_line = (uint32_t*)(GFX_BUFFER + (dst_y + row) * vm->pitch) + dst_x;
		uint32_t* src_px = (uint32_t*)src_line;
		uint32_t* dst_px = (uint32_t*)dst_line;
		for (size_t b = 0; b < width; ++b) *dst_px++ = *src_px++;
	}
}